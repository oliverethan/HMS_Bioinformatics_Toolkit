...
def parse_args():
    ...
    parser.add_argument('-in', '--input', help='path to file containing input')
    parser.add_argument('-out', '--output_file', help='file to which the outputs will be written to')
    parser.add_argument('-name', '--name', help='file to which the outputs will be written to')
...
def return_primary_command(args, output_file_name):
    primary_command = 'java' + ' -jar ' + args.picard_path + ' RenameSampleInVcf' ' \\' + '\n' + \
    '\t' + 'INPUT=' + args.input + ' \\' + '\n' + \
    '\t' + 'OUTPUT=' + args.output_file + ' \\' + '\n' + \
    '\t' + 'NEW_SAMPLE_NAME=' + args.name 
