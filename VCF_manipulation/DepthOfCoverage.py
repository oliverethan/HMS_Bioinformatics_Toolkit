...
def parse_args():
    ...
    parser.add_argument('-I', '--input_bam', help='path to input bam file')
    parser.add_argument('-out', '--output_name', help='file path to which the output will be written to')
    parser.add_argument('-n', '--num_cores', default='8', help='slurm job submission option')
    parser.add_argument('-t', '--runtime', default='5-0:00:00', help='slurm job submission option')
...
def clean_arg_paths(args):
    ...
    d = vars(args)
    for arg in d.keys():
        if 'output_name' in arg and d[arg]=='./': d[arg] = os.getcwd()
